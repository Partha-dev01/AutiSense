  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // FOSS build: no Bedrock route — fill from the local curated pools.
        if (IS_FOSS_BUILD) {
          if (!cancelled) {
            setSentences(pickFallback("sentences", ageMonths, 4));
            setInstructions(pickFallback("instructions", ageMonths, 3));
            setIsLoading(false);
          }
          return;
        }
