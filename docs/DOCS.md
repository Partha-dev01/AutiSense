...
| R59 | **Word Echo "no match" regression — re-fix of R57 (⚠️ do NOT restore the live mic visualizer)** | R58.1 above re-introduced the exact bug R57 fixed: restoring the `getUserMedia`-based MicVisualizer made a held mic stream monopolize the hardware on Chrome, so SpeechRecognition received zero audio again (visualizer bars moved, "Heard:" stayed empty, every word → "no match" across devices). Fix: re-applied R57 — removed `getUserMedia`/`acquireMic` and the `MicVisualizer` component entirely, release any mic stream + 500ms settle before `recognition.start()`, and replaced the live visualizer with CSS-animated listening bars so SpeechRecognition keeps **exclusive** mic access. Also kept/strengthened matching (full Levenshtein with length-scaled tolerance + looser prefix/substring) and added a gentle restart backoff (150→600ms) to stop mic thrash. **⚠️ The live-audio visualizer and SpeechRecognition are mutually exclusive on Chrome — never re-add a real `getUserMedia` visualizer to this page; keep the CSS bars.** Sibling pages (`kid-dashboard/speech`, `intake/audio`) were verified clean (no concurrent `getUserMedia`). `app/intake/communication/page.tsx` |
| R60 | **Word Echo accuracy: hybrid accept + higher sensitivity + honest visualizer** | After R59 recognition worked, but the Web Speech API transcribes young children unreliably (≈1/4 exact). By design decision (screening = "audio processing + speech production", not pronunciation grading): (1) **Hybrid accept** — a fuzzy word-match shows "Great match!"; if the child clearly spoke but it didn't match, accept ~1.4s after they pause (or at the hard timeout) and show "Nice try — heard you!". Pure noise does NOT pass (gated on `onspeechstart`/non-empty transcript, not raw sound). (2) **Higher sensitivity** — `maxAlternatives` 3→6, looser Levenshtein tolerance (~1 error per 2.5 chars). (3) **Honest visualizer** — the listening bars are now driven by the recognizer's own `onsoundstart`/`onsoundend`/`onspeechstart`/`onspeechend` events (`soundActive` state): they sit calm/grey and only turn red + animate when it actually hears the child — no fake "always-on" meter, still no `getUserMedia`. `app/intake/communication/page.tsx` |
