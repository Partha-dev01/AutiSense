...
| R54 | **Speech recognition fires "no match" too quickly on desktop** | SpeechRecognition `onend`/`onerror` would fire immediately if no speech was detected in the first ~500ms, jumping straight to "missed" state without giving the user time to speak. Fix: added 3-second minimum listen window (`MIN_LISTEN_MS = 3000`) across all audio-based pages. If `onend`/`onerror` fire before 3s, recognition is automatically restarted instead of marking missed. Applied to: Word Echo (`app/intake/communication/page.tsx`), Speech Practice (`app/kid-dashboard/speech/page.tsx`), Audio Intake (`app/intake/audio/page.tsx`). |
